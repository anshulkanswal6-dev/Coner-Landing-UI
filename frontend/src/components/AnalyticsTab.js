...
import { BarChart3, MessageSquare, Users, ThumbsUp, TrendingUp, Calendar } from "lucide-react";
...
export default function AnalyticsTab({ projectId }) {
  ...
  const fetchInsights = async () => {
    ...
    try {
      const now = new Date();
      const dateFrom = new Date(now.getTime() - parseInt(dateRange) * 24 * 60 * 60 * 1000).toISOString();
      const dateTo = now.toISOString();

      const res = await axios.get(`${API}/projects/${projectId}/analytics/insights/cards`, {
        ...
        {activeTab === "overview" && (
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-zinc-800/50 border border-white/10 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-purple-500"
            >
